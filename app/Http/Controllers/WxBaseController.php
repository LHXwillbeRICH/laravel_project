...
class WxBaseController extends Controller
{
    ...
    public function __construct($token = null){
        ...
        $a = explode('\\',$class);
        $name = array_pop($a);
        $this->Type = 'WX'.strtoupper(substr($name, 0, strlen($name)-10));
        //获取应用配置
        $config = config("services.{$this->Type}");
    ...
    public static function getInstance($type, $token = null) {
        ...
        $a=  require_once "sdk/{$name}.class.php";
        var_dump(class_exists($name));exit;
        if (class_exists($name)) {
            echo $name;exit;
            return new $name($token);
        } else {
            return new \Exception("实例化类文件{$name}失败");
        }
    }
